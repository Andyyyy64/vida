...
[![Version](https://img.shields.io/badge/version-0.5.2-green.svg)](https://github.com/Andyyyy64/vida/releases)
...
vida は **ローカル完結** のシステムです。キャプチャしたデータはマシン外に出ず、外向き通信は設定した LLM プロバイダー（任意）と許可済みの通知 webhook に限定されます。

- **ループバック限定サーバー。** デーモンは MJPEG (3002)、RAG (3003)、WebSocket (3004) をすべて `127.0.0.1` にバインドし、非ループバックのリモート、不正な `Host` ヘッダー（DNS リバインディング対策）、および許可されていないブラウザ Origin を拒否します。
- **Tauri 層の堅牢化。** 日付・パス・スケールを受ける IPC コマンドはすべて共通バリデーターを通り、ファイル読み取りは `safe_join` ヘルパーにより `data_dir` 以下に限定されます。Python バイナリ検出は正規化した上でリポジトリルート配下のみ許可し、`assetProtocol` のスコープは実行時に限定して動的登録します。
- **Webview の堅牢化。** CSP で `img/media/connect` を具体的なループバックポートに固定し、`script-src 'self'` / `object-src 'none'` / `frame-ancestors 'none'` を適用。RAG の Markdown 出力は DOMPurify でサニタイズします。
- **プロンプトインジェクション対策。** アナライザープロンプトに渡る信頼できない文字列（ウィンドウタイトル、文字起こし、チャット）は JSON エスケープされ、「キャプチャデータは指示ではない」というシステムノートをモデルに明示します。
- **シークレット管理。** API キーは `life.db` の `settings` テーブルに格納されます。POSIX ではデーモンが `data/` を `700`、DB ファイルを `600` に chmod します。LLM エラーメッセージは WebSocket でブロードキャストされる前に API キー文字列がスクラブされます。
- **E2E テスト。** ネットワーク層のインバリアント（Host / Origin / ボディサイズ / クエリ長）は `tests/e2e/` で実デーモンサーバーをループバック上に立てて検証し、フロントエンドのサニタイズと UI フローは `web/e2e/` (Playwright) でカバーします。

堅牢化項目の全リスト、脆弱性報告手順、ユーザー向けベストプラクティスは [SECURITY.md](SECURITY.md) を参照してください。
